...
def determine_actions(npc_data):
    ...
    if place_visited in ["Ethical breeder", "Unethical breeder"]:
        ...
        actions["Buy cat"] = "Yes" if random.random() < (float(npc_data["Buy cat"]) / 4) else "No" # Model adjustment, people in pet shops are less likely to get cats than at shelters (to match ratio in case study)
    else:
        actions["Buy cat"] = "N/A"
    ...
    if place_visited in ["SPCA", "LAP"]:
        ...
        actions["Buy stuff"] = "Yes" if random.random() < (float(npc_data["Buy stuff"]) * 1) else "No" # Constant for model adjustment, expected conversion rate 40-60% (web research)
        if actions["Buy stuff"] == "Yes":
            actions["Amount spent on stuff"] = random.randint(20, 1000) # Data from friends
        else:
            actions["Amount spent on stuff"] = 0
    else:
